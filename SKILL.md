...
- The `remotion-best-practices` skill for Phase-2 API/rendering knowledge (install from https://github.com/remotion-dev/skills if missing), and `remotion-motion-graphics` for the non-negotiable motion-design rules used on any bespoke `CustomGraphics.tsx` piece (install by cloning https://github.com/haidrrrry/claude-remotion-skill.git and copying its `remotion-motion-graphics/` folder into `~/.claude/skills/` if missing).
...
Both tracks: scaffold with one `cp -R` of the template, describe the video in `public/edit-data.json`, verify with montage stills, render, loudnorm, deliver `edit/final.mp4`. Load the `remotion-best-practices` skill for Remotion API/rendering mechanics. **For any bespoke piece in `CustomGraphics.tsx`** (brand intro, a new transition look, a one-off graphic no data field covers) **load `remotion-motion-graphics` too** and apply its non-negotiable rules — spring/easing never linear, 2-3 properties animating together, stagger on lists, a 5-layer stack (background → assets → graphics/text → grade → grain/vignette), Ken Burns on stills, idle-breathing after 2s, fps-derived timing, one `theme.ts` — then render → extract frames → inspect → fix before calling it done.
